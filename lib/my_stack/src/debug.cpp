...
void getStackState(LogLevel DepthMsg, stack* stk)
{
    ...
    if (DepthMsg >= 50)
    {
        int current_len = snprintf(serv_lines->list_data, SIZE_BUFFER,
                    "\tstack pointer = %p\n"
                    "\tCapacity: %zu\n"
                    "\tSize: %zd\n"
                    "\tData pointer: %p\n"
        ...
        for (size_t i = 0; i < stk->capacity + 2; i++)
        {
            ...
            {
                current_len += snprintf(serv_lines->list_data + current_len, SIZE_BUFFER - (size_t)current_len,
        ...
        snprintf(serv_lines->list_data + current_len, SIZE_BUFFER - (size_t)current_len, "\n\n");
    }
    ...
    else
    {
        ...
        if (GetLogger()->color_mode == COLOR_MODE)
        {
            current_len += snprintf(serv_lines->list_data, SIZE_BUFFER, COLOR_MAGENTA "\tData: " COLOR_RESET);
        }
        else
        {
            current_len += snprintf(serv_lines->list_data, SIZE_BUFFER, "\tData: ");
        }
